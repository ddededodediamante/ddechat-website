...
export default function Message({ data }) {
  ...
  if (!data?.author?.username && typeof data?.username === "string")
    data.author["username"] = data.username;
  if (!data?.author?.userId && typeof data?.userId === "string")
    data.author["userId"] = data.userId;

  let postContent = (
    <div className="vertical">
      {data?.author?.username && <p className="grey">@{data.author.username}</p>}
